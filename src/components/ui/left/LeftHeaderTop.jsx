...
import { ClickBox, ClickBoxItem } from "../ClickBox";
export function LeftHeaderTop({name}) {
  const [openBox, setOpenBox] = useState(false);

  return (
    <div className="flex justify-between px-2 items-start py-2 pr-3 border-r-2 border-[#2a353e]">
      <h1 className="text-white text-[1.4rem] mt-[4px] font-bold">{name}</h1>
      <div className="flex gap-4">
        ...
        >
          ...
          >
            ...
            <ClickBox className={`${openBox ? 'scale-100 z-10 rounded-md' : 'scale-0 -z-10' }`}>
              <ClickBoxItem>New group</ClickBoxItem>
              <ClickBoxItem>Starred messages</ClickBoxItem>
              <ClickBoxItem>Select chats</ClickBoxItem>
              <ClickBoxItem>Log out</ClickBoxItem>
            </ClickBox>
